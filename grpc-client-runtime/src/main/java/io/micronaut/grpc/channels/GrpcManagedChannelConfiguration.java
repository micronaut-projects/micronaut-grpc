...
public abstract class GrpcManagedChannelConfiguration implements Named {
    ...
    protected GrpcManagedChannelConfiguration(String name, Environment env, ExecutorService executorService) {
        this.name = name;
        connectOnStartup = env.getProperty(PREFIX + '.' + name + CONNECT_ON_STARTUP, Boolean.class).isPresent();
        connectionTimeout = env.getProperty(PREFIX + '.' + name + CONNECTION_TIMEOUT, Long.class)
            .filter(t -> t > 0)
        ...
        if (socketAddress.isPresent()) {
            ...
            if (serverAddress instanceof InetSocketAddress) {
                ...
                channelBuilder = NettyChannelBuilder.forAddress(isa.getHostName(), isa.getPort());
            } else {
                channelBuilder = NettyChannelBuilder.forAddress(serverAddress);
            }
        } else {
            final Optional<String> target = env.getProperty(PREFIX + '.' + name + SETTING_TARGET, String.class);
            if (target.isPresent()) {
                channelBuilder = NettyChannelBuilder.forTarget(
                    target.get()
                );
                ...
                if (uri != null && uri.getHost() != null && uri.getPort() > -1) {
                    ...
                    if (HttpRequest.SCHEME_HTTP.equalsIgnoreCase(uri.getScheme())) {
                        channelBuilder.usePlaintext();
                    }
                } else {
                    channelBuilder = NettyChannelBuilder.forTarget(name);
                }
            }
        }
