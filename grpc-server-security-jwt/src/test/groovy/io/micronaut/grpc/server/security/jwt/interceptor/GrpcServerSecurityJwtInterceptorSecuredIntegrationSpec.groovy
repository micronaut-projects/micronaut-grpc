...
@Ignore // FIXME: need to get test working with @Secured annotation
@MicronautTest(environments = ["greeter-hello-world-secured", Environment.TEST])
...
class GrpcServerSecurityJwtInterceptorSecuredIntegrationSpec extends Specification {

    @Inject
    private TestBean testBean

    def "test valid JWT with correct role works"() {
        expect:
        testBean.sayHelloWithJwt("Brian", "ROLE_ADMIN") == "Hello Brian"
    }

    def "test valid JWT with incorrect role does not work"() {
        when:
        testBean.sayHelloWithJwt("Brian", "ROLE_USER")
    ...
    @Factory
    @Requires(env = "greeter-hello-world-secured")
    ...
    static class TestBean {
        ...
        String sayHelloWithJwt(String message, String role) {
            UserDetails userDetails = new UserDetails("micronaut", [role])
    ...
    @Singleton
    static class SecuredGreeterImpl extends GreeterGrpc.GreeterImplBase {
