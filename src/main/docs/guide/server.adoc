...
To enable https://github.com/grpc/grpc/blob/master/doc/health-checking.md[gRPC health checks], add the following dependency to your build:

dependency:grpc-services[groupId="io.grpc"]

The health check will be enabled by default, but you can disable it with the following configuration:

.Disabling the gRPC health check.
[source,yaml]
----
grpc:
    server:
