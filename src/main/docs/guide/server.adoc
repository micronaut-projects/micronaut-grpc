...
[.lang-python]
--
To implement a gRPC server for the previously defined `helloworld.proto` definition you first need to generate the Java stubs using Gradle or Maven then create a class that implements `io.grpc.BindableService`. A Python class cannot extend the generated `GreeterGrpc.GreeterImplBase` class, instead it implements the methods of the generated `GreeterGrpc.AsyncService` interface and binds itself with `GreeterGrpc.bindService(self)`.
--
...
[.lang-python]
--
<1> The class implements `io.grpc.BindableService` and is annotated with `jakarta.inject.Singleton`
<2> You can dependency inject other beans into the implementation. In this case `GreetingService` is dependency injected.
<3> The `StreamObserver` is used to send a response to the client.
<4> `GreeterGrpc.bindService(self)` builds the `ServerServiceDefinition` of the service from the methods of the generated `GreeterGrpc.AsyncService` interface implemented by the class.
--
...
[.lang-python]
--
NOTE: A Python class cannot extend the generated Reactor base class (`ReactorReactiveGreeterGrpc.ReactiveGreeterImplBase`), so a Reactor gRPC service must be written in Java, Kotlin or Groovy. The generated Reactor client stub can be used from Python, see <<client, gRPC Clients>>.
--
...
[.lang-python]
--
NOTE: The Python listener customizes the `ServerBuilder` of the `GrpcServerConfiguration` bean, from which the `ServerBuilder` bean is created: the self-referencing generic type `ServerBuilder<T extends ServerBuilder<T>>` cannot be used as the type argument of a `BeanCreatedEventListener` implemented in Python.
--
...
snippet::helloworld.CustomInterceptor[tags="imports,clazz", source="main", languages="java,kotlin,groovy", title="Implement the io.micronaut.core.order.Ordered interface"]
...
[.lang-python]
--
NOTE: A Python class cannot implement `io.grpc.ServerInterceptor` directly (the `interceptCall` method is generic), so a Python interceptor is a plain object with an `interceptCall(call, headers, next)` method that is registered by wrapping it with `OrderedServerInterceptor` in a factory, as shown next.
--
